...
- **ClientSchema** - данные клиента (id, fio, email, phone)
- **ApartmentSchema** - данные квартиры (id, title, address)
- **BookingSchema** - данные бронирования (все поля модели)
- **WebhookDataSchema** - обертка для booking
- **WebhookPayloadSchema** - payload webhook (action, status, data)
- **WebhookRequestSchema** - полный запрос webhook
...
- **get_grouped_bookings()** - группирует бронирования по apartment_title и дате для отображения:
  ...
  - Определяет статус: Заселение, Выселение, Выс/Зас
  - Группирует дубли (например "00) 29" и "00) 29 ДУБЛЬ") вместе и размещает их рядом
  - Сортирует данные по дате и базовому адресу для правильного расположения дублей
...
- Кнопки: Фильтровать, Сбросить, Экспорт в Excel
...
- JavaScript для:
  - Форматирования чисел с разделителем тысяч (formatNumber)
  - Автосохранения комментариев с уведомлениями
  - Загрузки списка доступных услуг
  - Загрузки сумм услуг для всех бронирований (с форматированием)
  - Управления услугами через модальное окно
  - AJAX запросов для добавления/удаления услуг
